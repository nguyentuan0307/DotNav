...
<div class="toolbar"><button id="refresh" title="Refresh Git Log" aria-label="Refresh">↻</button><button data-action="fetch" title="Fetch all remotes and prune">Fetch</button><button data-action="update" title="Fetch and integrate upstream changes">Update</button><button data-action="pull" title="Pull current branch">Pull</button><button data-action="push" title="Push current branch">Push</button><button data-action="stash" title="Stash working tree changes">Stash</button><button data-action="createBranch" title="Create branch from HEAD">New Branch</button><button id="viewOptions" title="View options">View</button><span id="status"></span><span class="grow"></span><select id="repo" title="Repository"></select></div>
...
$('toastClose').onclick=hideToast;window.addEventListener('message',e=>{const m=e.data;if(m.type==='recovery'){showRecoveryToast(m);e.stopImmediatePropagation()}else if(m.type==='error'){showErrorToast(m.message);e.stopImmediatePropagation()}else if(m.type==='busy'){if(state.repository?.root&&m.repositoryId!==state.repository.root){e.stopImmediatePropagation();return}state.busy=m.busy;document.body.classList.toggle('busy',m.busy);$('status').textContent=m.busy?'Running '+actionLabel(m.action)+'…':panelStatus();e.stopImmediatePropagation()}},true);
...
function loadFiltered(){state.generation++;state.commits=[];state.total=0;state.loading.clear();state.loading.add(0);send('loadLog',{offset:0,generation:state.generation,filter:filter()})}let timer;for(const id of ['textFilter','authorFilter','pathFilter','sinceFilter','untilFilter'])$(id).oninput=()=>{clearTimeout(timer);timer=setTimeout(loadFiltered,250)};$('regex').onchange=$('case').onchange=loadFiltered;$('clear').onclick=()=>{for(const id of ['textFilter','authorFilter','pathFilter','sinceFilter','untilFilter'])$(id).value='';state.selectedRef=undefined;$('regex').checked=$('case').checked=false;renderBranches();$('status').textContent=panelStatus();loadFiltered()};
...
function showInlineContextMenu(message){const menu=$('contextMenu'),context=message.context;state.contextPayload=context;menu.innerHTML=message.actions.map(item=>'<button class="'+contextClass(item.action)+'" role="menuitem" data-context-action="'+esc(item.action)+'">'+esc(item.label)+'</button>').join('');if(!message.actions.length)return;menu.style.display='block';menu.style.left='0';menu.style.top='0';const point={x:context.x??state.contextPoint?.x??0,y:context.y??state.contextPoint?.y??0},rect=menu.getBoundingClientRect();menu.style.left=Math.max(4,Math.min(point.x,window.innerWidth-rect.width-4))+'px';menu.style.top=Math.max(4,Math.min(point.y,window.innerHeight-rect.height-4))+'px';menu.querySelector('button')?.focus()}
$('contextMenu').onclick=e=>{const button=e.target.closest('[data-context-action]');if(!button)return;const {type,...context}=state.contextPayload;send('contextAction',{...context,action:button.dataset.contextAction});$('contextMenu').style.display='none'};window.addEventListener('message',e=>{const m=e.data;if(m.type==='contextMenu')showInlineContextMenu(m);if(m.type==='busy'){state.busy=m.busy;document.body.classList.toggle('busy',m.busy);if(m.busy)$('status').textContent='Running '+m.action+'…'}if(m.type==='error'){const toast=$('toast');toast.textContent=m.message;toast.style.display='block';clearTimeout(state.toastTimer);state.toastTimer=setTimeout(()=>toast.style.display='none',8000)}if(m.type==='state'&&m.repository?.operation)setTimeout(()=>{const conflicts=m.uncommitted.filter(file=>file.conflict);$('operation').textContent=m.repository.operation+' · '+conflicts.length+' unresolved';document.querySelector('[data-conflict="continue"]').disabled=conflicts.length>0;document.querySelector('[data-conflict="skip"]').style.display=/REBAS|CHERRY/.test(m.repository.operation)?'inline-block':'none'},0)},true);
