...
<script nonce="${nonce}">
const vscode=acquireVsCodeApi(), ROW=28, PAGE=200, overscan=12;let state={commits:[],total:0,loading:new Set(),selected:-1,selectedHashes:new Set(),detail:null,uncommitted:[],favorites:new Set(JSON.parse(localStorage.getItem('gitLog.favorites')||'[]')),collapsed:new Set(JSON.parse(localStorage.getItem('gitLog.collapsed')||'[]'))};
