...
<div class="split" data-side="right"></div><section class="pane right"><div class="heading">CHANGED FILES <button id="fileMode" title="Toggle tree or flat view">Tree</button><select id="parentMode" style="display:none" title="Merge comparison parent"></select></div><div id="files"></div><div class="detail" id="detail"><div class="empty">Select a commit</div></div></section></main>
<script nonce="${nonce}">
const vscode=acquireVsCodeApi(), ROW=28, PAGE=200, overscan=12;let state={commits:[],total:0,loading:new Set(),selected:-1,selectedHashes:new Set(),detail:null,uncommitted:[],fileMode:localStorage.getItem('gitLog.fileMode')||'tree',favorites:new Set(JSON.parse(localStorage.getItem('gitLog.favorites')||'[]')),collapsed:new Set(JSON.parse(localStorage.getItem('gitLog.collapsed')||'[]'))};
...
function renderRows(){const vp=$('viewport'),start=Math.max(0,Math.floor(vp.scrollTop/ROW)-overscan),end=Math.min(state.total,Math.ceil((vp.scrollTop+vp.clientHeight)/ROW)+overscan);$('spacer').style.height=(state.total*ROW)+'px';$('spacer').innerHTML=Array.from({length:Math.max(0,end-start)},(_,i)=>{const n=start+i,c=state.commits[n];if(!c)return '<div class="row" style="top:'+(n*ROW)+'px"><span></span><span>Loading…</span></div>';const refs=c.refs.length?'<span class="refs">'+esc(c.refs[0].replace('refs/heads/','').replace('refs/remotes/','').replace('tag: refs/tags/',''))+'</span>':'';return '<div class="row '+(n===state.selected?'selected ':'')+(state.selectedHashes.has(c.hash)?'multi':'')+'" data-index="'+n+'" style="top:'+(n*ROW)+'px"><span class="graph">'+esc(c.graph)+'</span><span>'+esc(c.subject)+refs+'</span><span>'+esc(c.author)+'</span><span>'+date(c.authorTimestamp)+'</span></div>'}).join('');for(let page=Math.floor(start/PAGE)*PAGE;page<end;page+=PAGE)if(!state.commits[page]&&!state.loading.has(page)){state.loading.add(page);send('loadLog',{offset:page,filter:filter()})}}
function fileRow(f,depth=0,working=false){return '<div class="file" style="padding-left:'+(8+depth*14)+'px" data-path="'+esc(f.path)+'" '+(working?'data-working="true"':'')+'><b>'+esc(f.status)+'</b><span>'+esc(state.fileMode==='flat'?f.path:f.path.split('/').pop())+(state.fileMode==='tree'&&f.path.includes('/')?' <small>'+esc(f.path.slice(0,f.path.lastIndexOf('/')))+'</small>':'')+'</span><span class="stat">+'+f.additions+' -'+f.deletions+'</span></div>'}
function renderFiles(files,working=false){if(state.fileMode==='flat'){$('files').innerHTML=files.map(f=>fileRow(f,0,working)).join('');return}const folders=new Set();for(const f of files){const parts=f.path.split('/');for(let i=1;i<parts.length;i++)folders.add(parts.slice(0,i).join('/'))}let html=[...folders].sort().map(folder=>'<div class="item folder">▾ '+esc(folder)+'</div>').join('');html+=files.slice().sort((a,b)=>a.path.localeCompare(b.path)).map(f=>fileRow(f,f.path.split('/').length-1,working)).join('');$('files').innerHTML=html}
