...
- search commit message hoặc commit hash, luôn không phân biệt hoa/thường và highlight phần khớp;
- chọn một hoặc nhiều author từ dropdown có search; nhiều author dùng điều kiện OR; danh sách tích lũy từ các trang commit đã tải và chỉ tìm lịch sử cũ khi user search;
- chọn file hoặc folder đang tồn tại trong working tree local từ repository tree có fuzzy search;
- date preset Today/7 days/30 days hoặc custom range; ngày kết thúc bao gồm toàn bộ ngày;
- một hoặc nhiều branch/ref làm phạm vi lịch sử.
