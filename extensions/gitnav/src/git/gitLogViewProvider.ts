...
export class GitLogViewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
  ...
  private async handle(message: WebviewMessage): Promise<void> {
    ...
    try {
      ...
      const recovery = classifyGitError(text, message.action);
      ...
      this.post(recovery ? { type: 'recovery', recovery, operation: 'CHERRY-PICKING', request: { ref: message.ref, hash: message.hash, path: message.path } } : { type: 'error', message: text, scope: message.type });
    }
  }
...
.context-menu{max-height:calc(100vh - 8px);overflow:auto}.context-submenu{z-index:1001}.recovery-remember{display:flex;flex:1 0 100%;align-items:center;gap:7px;margin-bottom:2px}.recovery-remember input{width:16px;height:16px}
</style></head><body>
...
function showRecoveryToast(message){const recovery=message.recovery,toast=$('toast');$('toastTitle').textContent=recovery.title;$('toastMessage').textContent=recovery.kind==='pushRejected'?'Choose how to integrate origin before GitNav pushes again. Both choices preserve your local changes.':'Choose how to continue the current operation.';$('toastDetailText').textContent=recovery.detail;$('toastDetails').style.display='block';$('toastDetails').open=false;$('toastActions').innerHTML='';let remember;if(recovery.kind==='pushRejected'){remember=document.createElement('label');remember.className='recovery-remember';remember.innerHTML='<input type="checkbox"> Remember this choice for this repository';$('toastActions').appendChild(remember)}for(const item of recovery.actions){const button=document.createElement('button');button.textContent=item.label;button.onclick=()=>{hideToast();send('mutate',{action:item.action,strategy:item.strategy,remember:remember?.querySelector('input').checked===true,operation:message.operation,...(message.request||{})})};$('toastActions').appendChild(button)}toast.classList.add('recovery');toast.classList.remove('success');toast.style.display='block'}
$('toastClose').onclick=hideToast;window.addEventListener('message',e=>{const m=e.data;if(m.type==='recovery'){showRecoveryToast(m);e.stopImmediatePropagation()}else if(m.type==='error'){showErrorToast(m.message);if(['ready','refresh','loadLog'].includes(m.scope)&&!state.commits.length){$('emptyState').hidden=false;$('emptyState').innerHTML='<b>Unable to load Git history</b><span>Review the error details, then try again.</span><button data-empty-action="refresh">Try again</button>'}e.stopImmediatePropagation()}else if(m.type==='busy'){if(state.repository?.root&&m.repositoryId!==state.repository.root){e.stopImmediatePropagation();return}state.busy=m.busy;document.body.classList.toggle('busy',m.busy);for(const button of document.querySelectorAll('.toolbar [data-action]')){if(!button.dataset.idleTitle)button.dataset.idleTitle=button.title;const running=m.busy&&button.dataset.action===m.action;button.classList.toggle('running',running);button.title=running?(m.actionLabel||actionLabel(m.action))+' running…':button.dataset.idleTitle}if(m.busy)$('repoBadges').innerHTML='<span class="repo-badge operation">Running '+esc(m.actionLabel||actionLabel(m.action))+'…</span>';else{renderStatusBadges();if(m.succeeded&&m.applied)showActionFeedback(m.feedback||'status',m.actionLabel||actionLabel(m.action))}e.stopImmediatePropagation()}},true);
