...
interface WebviewMessage { type: string; root?: string; hash?: string; hashes?: string[]; path?: string; ref?: string; action?: string; kind?: string; current?: boolean; operation?: string; durationMs?: number; parent?: number; offset?: number; x?: number; y?: number; requestId?: number; generation?: number; filter?: GitLogFilter; plan?: GitRebasePlanItem[]; }
...
let state={commits:[],commitIndexes:new Map(),commitsByHash:new Map(),graphMaxColumn:0,total:0,hasMore:false,generation:0,busy:false,contextRequestId:0,loading:new Set(),selected:-1,selectionAnchor:-1,selectedHashes:new Set(),detail:null,uncommitted:[],showingUncommitted:false,visibleFiles:[],visibleFilesWorking:false,fileFolders:new Set(),fileMode:localStorage.getItem('gitLog.fileMode')||'tree',fileCollapsed:new Set(storedArray('gitLog.fileCollapsed')),favorites:new Set(storedArray('gitLog.favorites')),recentBranches:storedArray('gitLog.recentBranches'),collapsed:new Set(storedArray('gitLog.collapsed')),protectedBranches:[]};
