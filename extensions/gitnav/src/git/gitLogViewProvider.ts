...
.header,.row{grid-template-columns:max(var(--graph-width,70px),var(--graph-min-width,56px)) minmax(120px,1fr) var(--author-width,130px) var(--date-width,145px)}.header>span{position:relative;height:100%;display:flex;align-items:center}.column-resizer{position:absolute;z-index:4;top:0;right:-4px;width:8px;height:100%;cursor:col-resize;touch-action:none}.column-resizer:hover,.column-resizer.dragging{background:var(--vscode-sash-hoverBorder)}body.resizing-columns{cursor:col-resize;user-select:none}@media(max-width:900px){.header,.row{grid-template-columns:max(var(--graph-width,64px),var(--graph-min-width,56px)) minmax(120px,1fr) var(--author-width,120px)}.column-resizer[data-resize="author"]{display:none}}@media(max-width:700px){.header,.row{grid-template-columns:max(var(--graph-width,60px),var(--graph-min-width,56px)) minmax(120px,1fr)}.column-resizer[data-resize="subject"]{display:none}}
...
for(const handle of document.querySelectorAll('.column-resizer'))handle.onpointerdown=e=>{if(e.button!==0)return;e.preventDefault();e.stopPropagation();const cells=[...document.querySelectorAll('.header>span')],kind=handle.dataset.resize,start=e.clientX,graph=cells[0].getBoundingClientRect().width,author=cells[2].getBoundingClientRect().width,date=cells[3].getBoundingClientRect().width,center=document.querySelector('.center').clientWidth;handle.classList.add('dragging');document.body.classList.add('resizing-columns');handle.setPointerCapture(e.pointerId);handle.onpointermove=m=>{const delta=m.clientX-start;if(kind==='graph')setColumnWidth('graph',Math.max(56,Math.min(center-author-date-120,graph+delta)));else if(kind==='subject')setColumnWidth('author',Math.max(80,Math.min(center-56-date-120,author-delta)));else{const adjusted=Math.max(-(author-80),Math.min(date-105,delta));setColumnWidth('author',author+adjusted);setColumnWidth('date',date-adjusted)}};handle.onpointerup=handle.onpointercancel=()=>{handle.onpointermove=handle.onpointerup=handle.onpointercancel=null;handle.classList.remove('dragging');document.body.classList.remove('resizing-columns');saveColumnWidths()}};
