  ...
  private async handle(message: WebviewMessage): Promise<void> {
    ...
    try {
      ...
      if (message.type === 'searchAuthors' && message.query?.trim()) {
        ...
        try {
          const authors = await this.service.searchAuthors(root, message.query.trim());
          if (this.root === root) this.post({ type: 'filterAuthors', authors, repositoryId: root });
        } catch (error) {
...
<section class="pane center"><div class="banner" id="banner"><b id="operation"></b><button data-conflict="continue">Continue</button><button data-conflict="abort">Abort</button><button data-conflict="skip">Skip</button></div><div class="filters" id="filters" role="dialog" aria-label="Commit filters"><div class="filter-grid"><label class="filter-field author-filter">Authors<button class="filter-control" id="authorTrigger"><span id="authorSummary">Anyone</span></button></label><label class="filter-field path-filter">File or folder<button class="filter-control" id="pathTrigger"><span id="pathSummary">Any file</span></button></label><label class="filter-field date-filter">Date<select id="datePreset"><option value="any">Any time</option><option value="today">Today</option><option value="7">Last 7 days</option><option value="30">Last 30 days</option><option value="custom">Custom range</option></select></label><div class="filter-picker" id="authorPicker"><input id="authorSearch" placeholder="Search name or email…"><div id="authorChoices"></div></div><div class="filter-picker" id="pathPicker"><input id="pathSearch" placeholder="Search repository files…"><div id="pathChoices"></div></div><div class="date-custom" id="customDateRange"><label class="filter-field">From<input id="sinceFilter" type="date"></label><label class="filter-field">To<input id="untilFilter" type="date"></label></div></div><div class="match-toggle"><span>Match</span><button id="matchAll" class="active">All conditions</button><button id="matchAny">Any condition</button></div><div class="filter-footer"><small>Changes apply when this popup closes.</small><button id="clear">Clear filters</button></div></div><div class="column-menu" id="columnMenu" role="menu" aria-label="Commit columns"></div><div class="filter-chips" id="filterChips"></div><div class="header"><span data-col="graph">Graph<i class="column-resizer" data-resize="graph" title="Resize Graph column"></i></span><span data-col="subject">Commit<i class="column-resizer" data-resize="subject" title="Resize Commit and Author columns"></i></span><span data-col="author">Author<i class="column-resizer" data-resize="author" title="Resize Author and Date columns"></i></span><span data-col="date">Date</span></div><div class="row" id="uncommitted" style="display:none;position:relative"><span data-col="graph"></span><strong data-col="subject">Uncommitted changes</strong><span data-col="author"></span><span data-col="date"></span></div><div class="viewport" id="viewport" tabindex="0"><div class="empty-state" id="emptyState"><b>Loading Git history…</b><span>Reading commits and branches</span></div><div class="history-loading" id="historyLoading"><span>◌</span><span>Loading history…</span></div><div class="graph-clip" id="graphClip" aria-hidden="true"><svg class="graph-overlay" id="graphSvg"></svg></div><div class="spacer" id="spacer"></div></div></section>
...
function renderFilterChips(){const chips=activeFilters();$('filterChips').innerHTML=chips.map(x=>'<button class="filter-chip" data-clear-filter="'+x.id+'">'+esc(x.label)+' ×</button>').join('');$('toggleFilters').dataset.count=String(chips.filter(x=>x.id!=='ref').length)}
...
function fileRow(f,depth=0,working=false){const status=fileStatus(f),parts=f.path.split('/'),name=parts.pop()||f.path,folder=parts.join('/'),old=f.oldPath?' ← '+f.oldPath:'',path=state.fileMode==='flat'?folder+old:old,stats=(f.additions?'<span class="file-add">+'+f.additions+'</span>':'')+(f.deletions?'<span class="file-del">−'+f.deletions+'</span>':'');return '<div class="file '+status.css+(state.selectedFilePath===f.path?' selected':'')+'" role="treeitem" aria-selected="'+(state.selectedFilePath===f.path)+'" title="'+esc(status.label+': '+f.path+old)+'" style="padding-left:'+(8+depth*14)+'px" data-path="'+esc(f.path)+'" '+(working?'data-working="true" ':'')+(f.conflict?'data-conflict="true"':'')+'><span class="file-status" aria-label="'+status.label+'">'+status.code+'</span><span class="file-name">'+esc(name)+'</span><span class="file-path">'+esc(path)+'</span><span class="file-stat">'+stats+'</span><button class="file-open-local" data-open-local title="Open local file (Ctrl+Enter)" aria-label="Open local file">↗</button></div>'}
...
function renderAuthorChoices(){const q=$('authorSearch').value.trim().toLowerCase(),selected=new Set(state.filterDraft.authors);$('authorChoices').innerHTML=(state.filterOptions.authors||[]).filter(a=>!q||(a.name+' '+a.email).toLowerCase().includes(q)).map(a=>'<button class="filter-choice '+(selected.has(a.email)?'selected':'')+'" data-author="'+esc(a.email)+'"><span class="choice-check">'+(selected.has(a.email)?'✓':'')+'</span><span>'+esc(a.name)+'</span><small>'+esc(a.email)+'</small></button>').join('')||'<div class="empty">No matching authors</div>'}
function filterPaths(){const q=$('pathSearch').value.trim().toLowerCase(),files=state.filterOptions.files||[],folders=new Set();for(const file of files){const parts=file.split('/');for(let i=1;i<parts.length;i++)folders.add(parts.slice(0,i).join('/'))}return[...folders].map(path=>({path,folder:true})).concat(files.map(path=>({path,folder:false}))).filter(x=>!q||x.path.toLowerCase().includes(q)||fuzzyMatch(pathBase(x.path),q)).sort((a,b)=>a.path.localeCompare(b.path)).slice(0,300)}
function renderPathChoices(){$('pathChoices').innerHTML='<button class="filter-choice '+(!state.filterDraft.path?'selected':'')+'" data-path-choice=""><span>×</span><span>Any file</span></button>'+filterPaths().map(x=>'<button class="filter-choice '+(state.filterDraft.path===x.path?'selected':'')+'" style="padding-left:'+(7+(x.path.split('/').length-1)*12)+'px" data-path-choice="'+esc(x.path)+'"><span>'+(x.folder?'▸':'·')+'</span><span>'+esc(pathBase(x.path))+'</span><small>'+esc(x.path)+'</small></button>').join('')}
...
function renderFilterControls(){const f=state.filterDraft,authors=state.filterOptions.authors||[],names=f.authors.map(email=>authors.find(a=>a.email===email)?.name||email);$('authorSummary').textContent=names.length?(names[0]+(names.length>1?' +'+(names.length-1):'')):'Anyone';$('pathSummary').textContent=f.path||'Any file';$('sinceFilter').value=f.since||'';$('untilFilter').value=f.until||'';$('datePreset').value=inferDatePreset();$('customDateRange').classList.toggle('visible',$('datePreset').value==='custom');$('matchAll').classList.toggle('active',f.match!=='any');$('matchAny').classList.toggle('active',f.match==='any');renderAuthorChoices();renderPathChoices()}
function openFilters(){state.filterDraft={authors:[...(state.activeFilter.authors||[])],path:state.activeFilter.path||'',since:state.activeFilter.since||'',until:String(state.activeFilter.until||'').slice(0,10)};renderFilterControls();$('filters').classList.add('expanded');$('toggleFilters').setAttribute('aria-expanded','true')}
function closeFilters(apply=true){if(!$('filters').classList.contains('expanded'))return;$('filters').classList.remove('expanded');$('authorPicker').classList.remove('open');$('pathPicker').classList.remove('open');$('toggleFilters').setAttribute('aria-expanded','false');if(apply){const next={authors:state.filterDraft.authors.length?state.filterDraft.authors:undefined,path:state.filterDraft.path||undefined,since:state.filterDraft.since||undefined,until:state.filterDraft.until?state.filterDraft.until+' 23:59:59':undefined};const current={...state.activeFilter,text:undefined,refs:undefined};if(JSON.stringify(next)!==JSON.stringify(current)){state.activeFilter=next;loadFiltered()}}}
$('toggleFilters').onclick=e=>{e.stopPropagation();$('filters').classList.contains('expanded')?closeFilters(true):openFilters()};$('authorTrigger').onclick=()=>{$('pathPicker').classList.remove('open');$('authorPicker').classList.toggle('open');$('authorSearch').focus()};$('pathTrigger').onclick=()=>{$('authorPicker').classList.remove('open');$('pathPicker').classList.toggle('open');$('pathSearch').focus()};$('authorSearch').oninput=renderAuthorChoices;$('pathSearch').oninput=renderPathChoices;$('authorChoices').onclick=e=>{const button=e.target.closest('[data-author]');if(!button)return;const email=button.dataset.author,index=state.filterDraft.authors.indexOf(email);index>=0?state.filterDraft.authors.splice(index,1):state.filterDraft.authors.push(email);renderFilterControls()};$('pathChoices').onclick=e=>{const button=e.target.closest('[data-path-choice]');if(!button)return;state.filterDraft.path=button.dataset.pathChoice;$('pathPicker').classList.remove('open');renderFilterControls()};$('datePreset').onchange=e=>{const value=e.target.value;if(value!=='custom')Object.assign(state.filterDraft,presetRange(value));$('customDateRange').classList.toggle('visible',value==='custom');renderFilterControls()};$('sinceFilter').onchange=e=>state.filterDraft.since=e.target.value;$('untilFilter').onchange=e=>state.filterDraft.until=e.target.value;$('matchAll').onclick=()=>{state.filterDraft.match='all';renderFilterControls()};$('matchAny').onclick=()=>{state.filterDraft.match='any';renderFilterControls()};$('clear').onclick=clearFilters;document.addEventListener('pointerdown',e=>{if($('filters').classList.contains('expanded')&&!$('filters').contains(e.target)&&!$('toggleFilters').contains(e.target))closeFilters(true)});
