...
.layout{grid-template-columns:44px var(--left,220px) 4px minmax(320px,1fr) 4px var(--right,330px)}.quick-actions{grid-column:1;grid-row:1;border-left:0;border-right:1px solid var(--vscode-panel-border)}.branches{grid-column:2;grid-row:1}.split[data-side="left"]{grid-column:3;grid-row:1}.center{grid-column:4;grid-row:1}.split[data-side="right"]{grid-column:5;grid-row:1}.right{grid-column:6;grid-row:1}@media(max-width:900px){.layout{grid-template-columns:42px var(--left,180px) 3px minmax(260px,1fr) 3px var(--right,260px)}}
</style></head><body>
...
let state={commits:[],commitIndexes:new Map(),commitsByHash:new Map(),graphMaxColumn:0,total:0,generation:0,busy:false,contextRequestId:0,loading:new Set(),selected:-1,selectionAnchor:-1,selectedHashes:new Set(),detail:null,uncommitted:[],visibleFiles:[],visibleFilesWorking:false,fileFolders:new Set(),fileMode:localStorage.getItem('gitLog.fileMode')||'tree',fileCollapsed:new Set(storedArray('gitLog.fileCollapsed')),favorites:new Set(storedArray('gitLog.favorites')),collapsed:new Set(storedArray('gitLog.collapsed'))};
...
$('viewOptions').onclick=()=>{$('toggleColumns').click()};
function closeColumnMenu(){$('columnMenu').classList.remove('open');$('toggleColumns').setAttribute('aria-expanded','false')}
function positionColumnMenu(){const button=$('toggleColumns'),menu=$('columnMenu'),anchor=button.getBoundingClientRect(),bounds=menu.getBoundingClientRect(),gap=4;menu.style.left=Math.max(gap,Math.min(anchor.right-bounds.width,window.innerWidth-bounds.width-gap))+'px';menu.style.top=Math.max(gap,Math.min(anchor.bottom+gap,window.innerHeight-bounds.height-gap))+'px'}
...
document.addEventListener('contextmenu',e=>{e.preventDefault();state.contextPoint={x:e.clientX,y:e.clientY}},true);document.addEventListener('click',e=>{if(!e.target.closest('#contextMenu'))$('contextMenu').style.display='none';if(!e.target.closest('#columnMenu,#toggleColumns'))closeColumnMenu()});window.addEventListener('blur',()=>{$('contextMenu').style.display='none';closeColumnMenu()});window.addEventListener('resize',()=>{if($('columnMenu').classList.contains('open'))positionColumnMenu()});document.addEventListener('keydown',e=>{if(e.key==='Escape'){$('contextMenu').style.display='none';closeColumnMenu()}});
...
for(const split of document.querySelectorAll('.split'))split.onmousedown=e=>{const side=split.dataset.side,start=e.clientX,layout=$('layout'),initial=side==='left'?layout.children[0].offsetWidth:layout.children[5].offsetWidth;let frame,latest=start,value=initial;document.body.classList.add('resizing');const apply=()=>{frame=undefined;value=Math.max(140,initial+(side==='left'?latest-start:start-latest));layout.style.setProperty('--'+side,value+'px')};document.onmousemove=m=>{latest=m.clientX;if(!frame)frame=requestAnimationFrame(apply)};document.onmouseup=()=>{if(frame){cancelAnimationFrame(frame);apply()}localStorage.setItem('gitLog.'+side,String(value));document.body.classList.remove('resizing');document.onmousemove=document.onmouseup=null;const range=visibleRange();renderGraph(range.start,range.end)}};for(const side of ['left','right']){const v=localStorage.getItem('gitLog.'+side);if(v)$('layout').style.setProperty('--'+side,v+'px')}
const savedDetail=localStorage.getItem('gitLog.detailHeight');if(savedDetail)$('rightPane').style.setProperty('--detail-height',savedDetail+'px');$('rightSplit').onmousedown=e=>{const pane=$('rightPane'),start=e.clientY,initial=$('detail').offsetHeight;let frame,latest=start,height=initial;document.body.classList.add('resizing');const apply=()=>{frame=undefined;height=Math.max(90,Math.min(pane.clientHeight-90,initial+start-latest));pane.style.setProperty('--detail-height',height+'px')};document.onmousemove=m=>{latest=m.clientY;if(!frame)frame=requestAnimationFrame(apply)};document.onmouseup=()=>{if(frame){cancelAnimationFrame(frame);apply()}localStorage.setItem('gitLog.detailHeight',String(height));document.body.classList.remove('resizing');document.onmousemove=document.onmouseup=null;const range=visibleRange();renderGraph(range.start,range.end)}};for(const tab of document.querySelectorAll('[data-right-tab]'))tab.onclick=()=>{$('rightPane').classList.toggle('mobile-files',tab.dataset.rightTab==='files');$('rightPane').classList.toggle('mobile-detail',tab.dataset.rightTab==='detail')};
window.onmessage=e=>{const m=e.data;if(m.type==='rebasePlan')showRebasePlan(m.plan);else if(m.type==='inlineDiff')renderInlineDiff(m.diff);else if(m.type==='selectRef'){state.selectedRef=m.ref;renderBranches();loadFiltered()}else if(m.type==='state'){state={...state,...m,commits:m.log?.commits??[],total:m.log?.total??0};state.commits.length=state.total;indexCommits(m.log?.commits??[],0,true);$('repo').style.display=m.repositories.length>1?'block':'none';$('repo').innerHTML=m.repositories.map(r=>'<option '+(r===m.repository?.root?'selected':'')+'>'+esc(r)+'</option>').join('');const r=m.repository;renderStatusBadges();$('uncommitted').style.display=state.uncommitted.length?'grid':'none';$('banner').style.display=r?.operation?'block':'none';$('operation').textContent=r?.operation??'';renderBranches();renderFilterChips();renderRows();if(r?.operation){$('files').innerHTML=state.uncommitted.filter(f=>f.conflict).map(f=>fileRow(f,0,true)).join('')}}else if(m.type==='log'){state.total=m.log.total;if(state.commits.length!==state.total)state.commits.length=state.total;state.commits.splice(m.log.offset,m.log.commits.length,...m.log.commits);indexCommits(m.log.commits,m.log.offset);state.loading.delete(m.log.offset);renderRows()}else if(m.type==='detail'){state.detail=m.detail;renderDetail()}else if(m.type==='compareFiles'){renderFiles(m.files);$('detail').innerHTML='<div class="message">'+esc(m.from)+' ↔ '+esc(m.to)+'</div><div class="meta">'+(m.onlyCurrent?m.onlyCurrent.length+' commit(s) only in current<br>'+m.onlySelected.length+' commit(s) only in selected<br>':'')+m.files.length+' changed file(s)</div>'}};send('ready');
</script></body></html>`;
}
