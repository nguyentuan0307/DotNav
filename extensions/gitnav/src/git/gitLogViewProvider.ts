...
function renderFiles(files,working=false){state.visibleFiles=files;state.visibleFilesWorking=working;if(!files.some(file=>file.path===state.selectedFilePath))state.selectedFilePath='';state.fileFolders.clear();const additions=files.reduce((sum,f)=>sum+f.additions,0),deletions=files.reduce((sum,f)=>sum+f.deletions,0);$('fileSummary').innerHTML=files.length?'<span class="summary-count">'+files.length+'</span><span class="summary-add">+'+additions+'</span><span class="summary-del">−'+deletions+'</span>':'';$('treeMode').classList.toggle('active',state.fileMode==='tree');$('listMode').classList.toggle('active',state.fileMode==='flat');if(!files.length){$('files').innerHTML='<div class="empty">'+(state.detail?'This commit does not change any files.':'Select a commit to view changed files.')+'</div>';return}if(state.fileMode==='flat')$('files').innerHTML=files.map(f=>fileRow(f,0,working)).join('');else $('files').innerHTML=renderFileNode(fileTree(files),'',0,working);applyDepth($('files'))}
