...
</style><style>.heading>.grow{flex:1}.branches .left-pane-heading{height:44px;padding:6px 9px;gap:8px;background:var(--vscode-editor-background)}.left-mode-toggle{display:inline-flex;height:31px;align-items:stretch;overflow:hidden;border:1px solid var(--vscode-panel-border);border-radius:5px;background:var(--vscode-input-background)}.heading .left-mode-toggle button{width:auto;height:29px;padding:0 9px;border:0;border-right:1px solid var(--vscode-panel-border);background:transparent;color:var(--vscode-descriptionForeground);font-size:12px!important;font-weight:600!important;letter-spacing:0}.heading .left-mode-toggle button:last-child{border-right:0}.heading .left-mode-toggle button:hover{color:var(--vscode-foreground);background:var(--vscode-toolbar-hoverBackground)}.heading .left-mode-toggle button.active{color:var(--vscode-list-activeSelectionForeground);background:var(--vscode-list-activeSelectionBackground)}.stash-count{font-variant-numeric:tabular-nums}.stash-count:empty{display:none}#leftAdd{width:29px!important;height:29px!important;padding:0!important;border-radius:4px;font-size:17px!important;color:var(--vscode-foreground)}.stash-item{position:relative;display:grid!important;grid-template-columns:20px minmax(0,1fr);height:50px!important;padding-top:5px!important;padding-bottom:5px!important;align-items:center}.stash-copy{min-width:0;overflow:hidden}.stash-message,.stash-meta{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.stash-message{font-size:12px;line-height:20px}.stash-meta{color:var(--vscode-descriptionForeground);font-size:10px;line-height:15px}.stash-actions{position:absolute;right:5px;bottom:3px;display:inline-flex;visibility:hidden;align-items:center;gap:2px;padding:1px 0 1px 7px;background:var(--vscode-list-hoverBackground)}.stash-item:hover .stash-actions,.stash-item:focus-within .stash-actions{visibility:visible}.stash-item:hover .stash-meta,.stash-item:focus-within .stash-meta{visibility:hidden}.stash-action{width:auto!important;height:21px!important;padding:0 5px!important;border:0!important;border-radius:3px;background:transparent!important;color:var(--vscode-descriptionForeground);font-size:11px!important}.stash-action:hover,.stash-action:focus-visible{color:var(--vscode-foreground);background:var(--vscode-toolbar-hoverBackground)!important;outline:1px solid var(--vscode-focusBorder);outline-offset:-1px}.stash-action.danger{color:var(--vscode-errorForeground)}
</style></head><body>
...
<main class="layout" id="layout"><section class="pane branches"><div class="heading left-pane-heading"><span class="left-mode-toggle" role="group" aria-label="References view"><button data-left-mode="branches" aria-pressed="true">Branches</button><button data-left-mode="stashes" aria-pressed="false">Stashes<span class="stash-count" id="stashCount"></span></button></span><span class="grow"></span><button id="leftAdd" data-action="createBranch" title="Create branch">＋</button></div><input id="branchSearch" placeholder="Search branches"><div id="branches" role="listbox" aria-label="Branches and stashes"></div></section><div class="split" data-side="left"></div>
...
$('branches').onclick=e=>{const item=e.target.closest('.item');if(!item)return;if(item.dataset.kind==='stash'&&item.dataset.ref){state.selectedRef=state.selectedRef===item.dataset.ref?undefined:item.dataset.ref;renderBranches();loadFiltered();return}if(item.dataset.ref){state.selectedRef=item.dataset.ref;renderBranches();loadFiltered()}else if(item.dataset.hash)send('detail',{hash:item.dataset.hash})};
