...
function applyColumnVisibility(){const visible=COMMIT_COLUMNS.filter(c=>visibleColumns.has(c.id));document.documentElement.style.setProperty('--commit-grid',visible.map(c=>c.width).join(' '));document.querySelectorAll('[data-col]').forEach(el=>el.classList.toggle('commit-col-hidden',!visibleColumns.has(el.dataset.col)));$('graphSvg').style.display=visibleColumns.has('graph')?'':'none';renderColumnMenu()}
...
applyColumnVisibility();$('toggleColumns').onclick=e=>{e.stopPropagation();const open=$('columnMenu').classList.toggle('open');$('toggleColumns').setAttribute('aria-expanded',String(open))};$('columnMenu').onclick=e=>{e.stopPropagation();const input=e.target.closest('[data-column-toggle]');if(!input)return;const id=input.dataset.columnToggle;if(input.checked)visibleColumns.add(id);else if(visibleColumns.size>1)visibleColumns.delete(id);localStorage.setItem('gitLog.visibleColumns',JSON.stringify([...visibleColumns]));applyColumnVisibility()};for(const handle of document.querySelectorAll('.column-resizer'))handle.onpointerdown=e=>{if(e.button!==0)return;e.preventDefault();e.stopPropagation();const kind=handle.dataset.resize,start=e.clientX,cell=id=>document.querySelector('.header>[data-col="'+id+'"]')?.getBoundingClientRect().width||0,graph=cell('graph'),author=cell('author'),date=cell('date'),center=document.querySelector('.center').clientWidth;handle.classList.add('dragging');document.body.classList.add('resizing-columns');handle.setPointerCapture(e.pointerId);handle.onpointermove=m=>{const delta=m.clientX-start;if(kind==='graph')setColumnWidth('graph',Math.max(28,Math.min(center-author-date-120,graph+delta)));else if(kind==='subject')setColumnWidth('author',Math.max(80,Math.min(center-28-date-120,author-delta)));else{const adjusted=Math.max(-(author-80),Math.min(date-105,delta));setColumnWidth('author',author+adjusted);setColumnWidth('date',date-adjusted)}};handle.onpointerup=handle.onpointercancel=()=>{handle.onpointermove=handle.onpointerup=handle.onpointercancel=null;handle.classList.remove('dragging');document.body.classList.remove('resizing-columns');saveColumnWidths()}};
